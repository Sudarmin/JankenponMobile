package p_menuBar
{
	...
	public class TG_QuestionTypeC extends TG_MenuBar
	{
		...
		private var m_copyDone1:Boolean = false
		private var m_copyDone2:Boolean = false
		...
		public function TG_QuestionTypeC(parent:DisplayObjectContainer, gameState:TG_GameState)
		{
		...
		public final function startBattle():void
		{
			m_answers1 = [];
			m_answerCounter1 = 0;
			m_answers2 = [];
		...
		public function checkNextAnswer():int
		{
			...
			if(m_answerSprites1.length > m_resultCounter || m_answerSprites2.length > m_resultCounter)
			{
				var i:int = 0;
				var size:int = 0;
				var num:int = 0;
				...
				addABitAnimation(m_answerSprites1[m_resultCounter]);
				addABitAnimation(m_answerSprites2[m_resultCounter]);
				
				if(m_answers1[m_resultCounter] == null && m_answers2[m_resultCounter] == null)
				{
					addDrawIcon(m_answerSprites1[m_resultCounter]);
					addDrawIcon(m_answerSprites2[m_resultCounter]);
					m_resultCounter++;
					return 0;
				}
				//we check who wins
				if(m_answers1[m_resultCounter] == ROCK)
				{
					if(m_answers2[m_resultCounter] == ROCK)
					{
						addDrawIcon(m_answerSprites1[m_resultCounter]);
						addDrawIcon(m_answerSprites2[m_resultCounter]);
						m_resultCounter++;
					...
					else if(m_answers2[m_resultCounter] == PAPER)
					{
						addLoseIcon(m_answerSprites1[m_resultCounter]);
						m_resultCounter++;
						return 2;
					}
					else if(m_answers2[m_resultCounter] == SCISSOR)
					{
						addLoseIcon(m_answerSprites2[m_resultCounter]);
						m_resultCounter++;
						return 1;
					}
					else
				...
				else if(m_answers1[m_resultCounter] == PAPER)
				{
					if(m_answers2[m_resultCounter] == ROCK)
					{
						addLoseIcon(m_answerSprites2[m_resultCounter]);
				...
				else if(m_answers1[m_resultCounter] == SCISSOR)
				{
					if(m_answers2[m_resultCounter] == ROCK)
					{
						addLoseIcon(m_answerSprites1[m_resultCounter]);
			...
			if(m_resultCounter >= m_answerSprites1.length && m_resultCounter >= m_answerSprites2.length)
			{
				return -1;
			}
			
			//there's no answer waiting, so draw by default
			addDrawIcon(m_answerSprites1[m_resultCounter]);
			addDrawIcon(m_answerSprites2[m_resultCounter]);
			m_resultCounter++;
			return 0;
		...
		private function openAnswers1():void
		{
			var size:int = m_answers1.length;
			var i:int = 0;
			var image:Image;
			var sprite:Sprite;
			var prevSprite:Sprite;
			for(i;i<size;i++)
			{
				image = createImageRPS(m_answers1[i]);
				sprite = new Sprite();
				sprite.addChild(image);
				sprite.pivotX = sprite.width * 0.5;
				sprite.pivotY = sprite.height * 0.5;
				...
				m_sprite.addChild(sprite);
				m_answerSprites1[i] = sprite;
			}
		}
		
		private function openAnswers2():void
		{
			var size:int = m_answers2.length;
			var i:int = 0;
			var image:Image;
			var sprite:Sprite;
			var prevSprite:Sprite;
			for(i;i<size;i++)
			{
				image = createImageRPS(m_answers2[i]);
				sprite = new Sprite();
				sprite.addChild(image);
				sprite.pivotX = sprite.width * 0.5;
				sprite.pivotY = sprite.height * 0.5;
		...
		public function copyAnswers1():void
		{
			openAnswers1();
			var image:Image;
			var sprite:Sprite;
			var prevSprite:Sprite;
			var size:int = m_answerSprites1.length;
			var i:int = 0;
			for(i;i<size;i++)
			{
				sprite = m_answerSprites1[i];
				prevSprite = m_questionSprites[i];
				if(i == size-1)
				{
					TweenMax.to(sprite,0.5,{delay:i*0.1,onComplete:onCopyCompleted1,scaleX:prevSprite.scaleX,scaleY:prevSprite.scaleY,x:prevSprite.x,y:prevSprite.y,ease:Circ.easeOut});
				}
				else
				{
				...
				prevSprite.removeFromParent();
				m_questionSprites[i] = sprite;
			}
		}
		
		private function copyAnswers2():void
		{
			openAnswers2();
			var image:Image;
			var sprite:Sprite;
			var prevSprite:Sprite;
			var size:int = m_answerSprites2.length;
			var i:int = 0;
			for(i;i<size;i++)
			{
				sprite = m_answerSprites2[i];
				...
				if(i == size-1)
				{
					TweenMax.to(sprite,0.5,{delay:i*0.1,onComplete:onCopyCompleted2,scaleX:prevSprite.scaleX,scaleY:prevSprite.scaleY,x:prevSprite.x,y:prevSprite.y,ease:Circ.easeOut});
				}
				else
				{
					TweenMax.to(sprite,0.5,{delay:i*0.1,scaleX:prevSprite.scaleX,scaleY:prevSprite.scaleY,x:prevSprite.x,y:prevSprite.y,ease:Circ.easeOut});
		...
		private function onCopyCompleted1():void
		{
			m_copyDone1 = true;
			copyAnswers2();
		}
		private function onCopyCompleted2():void
		{
			m_copyDone2 = true;
		}
		
		public function get copyDone1():Boolean
		{
			return m_copyDone1;
		}
		
		public function get copyDone2():Boolean
		{
			return m_copyDone2;
		}
		...
		{
			if(m_answerCounter1 >= m_numOfQuestion && m_answerCounter2 >= m_numOfQuestion)
			{
				m_ready = false;
				return true;
			}
